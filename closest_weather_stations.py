...
def distance(entity, stations):
    ...
    h2 = np.cos(entity_lat)*np.cos(stations['LAT_RAD'])*hav(stations['LON_RAD']-entity_lon)
...
def closest_station(entity, stations):
    ...
    stations = stations[(stations['First Year'] <= first_year) & (stations['Last Year']>=last_year)]
    if len(stations) <= 0:
...
stations = stations[['Latitude (Decimal Degrees)','Longitude (Decimal Degrees)','LAT_RAD', 'LON_RAD', 'Name', 'Climate ID', 'Station ID', 'First Year', 'Last Year', 'Elevation (m)']]

# link up the best (closest station w/data) weather to hydro station
best_weather_stations = lakes_with_latlon.apply(closest_station, axis=1, stations=stations).set_index('HYDRO_ID')
weather_data = pd.merge(best_weather_stations, lakes_with_latlon.set_index('STATION_NUMBER'), left_index=True, right_index=True)
